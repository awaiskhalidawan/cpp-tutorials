...
int main()
{
    ...
    auto result = performDivision(10, 1);       // Call the function to perfrom the operation.

    if (result.has_value()) {                   // Check if the result has a valid value. Print the value.
        std::cout << "Result: " << result.value() << std::endl;
    }
    else {                      // If there are no valid values in the result than it contains and unexpected error value.
